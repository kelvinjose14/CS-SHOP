...
**Lo que está listo:**
- Todas las funciones pedidas: inventario, compras, ventas, clientes, cuentas, gastos, caja, contabilidad, dashboard, 15 reportes, usuarios e historial ([Requisitos](requisitos.md), 99 de 102 cumplen).
- 14 pruebas automáticas de la lógica del negocio, que corren en cada cambio (CI).
...
|---|---|---|---|
| 1 | **Funciona en una sola computadora** | No sirve para la tienda con varias PCs en red | O2 |
...
| 7 | La base se guarda completa en cada operación | Con años de datos puede volverse lento (sin medir) | O2, O3 |
...
**Estado:** en curso (este pull request).
...
**Estado:** pendiente. Requiere decidir antes DP-01, DP-02 y DP-03.

**Meta:** que dos o más computadoras de la tienda trabajen al mismo tiempo sobre los mismos datos.

**Propuesta a evaluar (DP-01):** una **computadora principal** guarda la base de datos y atiende a las demás por la red local de la tienda. Las otras computadoras usan el mismo programa en modo "conectado a la principal". No necesita internet ni tiene costo mensual.
...
2. **Base de datos con acceso concurrente**, en reemplazo de la base actual en memoria (sql.js), con **migración automática** de los datos de la versión 1.0.0.
3. **Sesiones por computadora:** cada PC con su usuario y su caja (según DP-02).
...
**Depende de:** O1, DP-01, DP-02 y DP-03.
